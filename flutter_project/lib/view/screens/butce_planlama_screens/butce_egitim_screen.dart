...
class ButceEgitim extends StatefulWidget {
  const ButceEgitim({Key? key}) : super(key: key);

  @override
  State<StatefulWidget> createState() => ButceEgitimState();
}

class ButceEgitimState extends State<ButceEgitim> {
                ...
                SizedBox(
                  ...
                  child: FutureBuilder(
                    future: _gelirService.getData(
                        Apis.getGelirApi, "6", startDate, endDate),
                              ...
                              child: InkWell(
                                onTap: () {
                                  _gelirWidget.showGelirDetails(context,
                                      document.id, "6", startDate, endDate);
                                },
                                child: Row(
                                  children: [
                                    Padding(
                  ...
                  children: <Widget>[
                    FutureBuilder(
                      future: _gelirService.getCategoriesForUser(
                          userId, "${Apis.getGelirCategoriesApi}6"),
                      builder: (BuildContext context,
  ...
  Future<void> _showDeleteSubCategoryDialog(int userId) async {
    ...
    return showDialog(
      context: context,
      builder: (BuildContext context) => AlertDialog(
        title: const Text("Eğitim Türü Kaldır"),
        content: SingleChildScrollView(
          child: Padding(
            padding: const EdgeInsets.all(8),
            child: Form(
            ...
            onPressed: () {
              setState(() {
                _gelirService.deleteSubCategory(context, userId, selectDataItem,
                    6 /*categoryId*/, Apis.deleteGelirSubCategoriesApi);
              });
            },
  ...
  Future<void> _showAddSubCategoryDialog(int userId) async {
    ...
    return showDialog(
        context: context,
        builder: (BuildContext context) => AlertDialog(
          title: const Text('Eğitim Türü Ekle'),
          content: SingleChildScrollView(
            child: Padding(
              padding: const EdgeInsets.all(8),
              child: Form(
                ...
                child: TextFormField(
                  decoration: const InputDecoration(
                    hintText: 'Eğitim Türü',
                  ),
                  validator: (String? value) {
                    if (value == null || value.isEmpty) {
                      return 'Lütfen Geçerli bir Eğitim Türü giriniz';
                    }
                    return null;
                  },
                  onChanged: (value) {
            ...
            TextButton(
                onPressed: () {
                  _gelirService.saveSubCategory(context, userId, gelirType,
                      Apis.saveGelirSubCategoriesApi, 6 /*categoryId*/);
                },
                child: const Text('Kaydet')),
            TextButton(
                onPressed: () {
