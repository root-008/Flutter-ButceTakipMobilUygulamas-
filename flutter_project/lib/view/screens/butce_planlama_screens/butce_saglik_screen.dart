...
class ButceSaglik extends StatefulWidget {
  const ButceSaglik({Key? key}) : super(key: key);

  @override
  State<StatefulWidget> createState() => ButceSaglikState();
}

class ButceSaglikState extends State<ButceSaglik> {
                ...
                SizedBox(
                  ...
                  child: FutureBuilder(
                    future: _gelirService.getData(
                        Apis.getGelirApi, "5", startDate, endDate),
                              ...
                              child: InkWell(
                                onTap: () {
                                  _gelirWidget.showGelirDetails(context,
                                      document.id, "5", startDate, endDate);
                                },
                                child: Row(
                                  children: [
                                    Padding(
                  ...
                  children: <Widget>[
                    FutureBuilder(
                      future: _gelirService.getCategoriesForUser(
                          userId, "${Apis.getGelirCategoriesApi}5"),
                      builder: (BuildContext context,
  ...
  Future<void> _showDeleteSubCategoryDialog(int userId) async {
    ...
    return showDialog(
      context: context,
      builder: (BuildContext context) => AlertDialog(
        title: const Text("Sağlık Türü Kaldır"),
        content: SingleChildScrollView(
          child: Padding(
            padding: const EdgeInsets.all(8),
            child: Form(
            ...
            onPressed: () {
              setState(() {
                _gelirService.deleteSubCategory(context, userId, selectDataItem,
                    5 /*categoryId*/, Apis.deleteGelirSubCategoriesApi);
              });
            },
  ...
  Future<void> _showAddSubCategoryDialog(int userId) async {
    ...
    return showDialog(
        context: context,
        builder: (BuildContext context) => AlertDialog(
          title: const Text('Sağlık Türü Ekle'),
          content: SingleChildScrollView(
            child: Padding(
              padding: const EdgeInsets.all(8),
              child: Form(
                ...
                child: TextFormField(
                  decoration: const InputDecoration(
                    hintText: 'Sağlık Türü',
                  ),
                  validator: (String? value) {
                    if (value == null || value.isEmpty) {
                      return 'Lütfen Geçerli bir Sağlık Türü giriniz';
                    }
                    return null;
                  },
                  onChanged: (value) {
            ...
            TextButton(
                onPressed: () {
                  _gelirService.saveSubCategory(context, userId, gelirType,
                      Apis.saveGelirSubCategoriesApi, 5 /*categoryId*/);
                },
                child: const Text('Kaydet')),
            TextButton(
                onPressed: () {
