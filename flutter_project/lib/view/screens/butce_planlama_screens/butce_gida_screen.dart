...
class ButceGida extends StatefulWidget {
  const ButceGida({Key? key}) : super(key: key);

  @override
  State<StatefulWidget> createState() => ButceGidaState();
}

class ButceGidaState extends State<ButceGida> {
                ...
                SizedBox(
                  ...
                  child: FutureBuilder(
                    future: _gelirService.getData(
                        Apis.getGelirApi, "2", startDate, endDate),
                              ...
                              child: InkWell(
                                onTap: () {
                                  _gelirWidget.showGelirDetails(context,
                                      document.id, "2", startDate, endDate);
                                },
                                child: Row(
                                  children: [
                                    Padding(
                  ...
                  children: <Widget>[
                    FutureBuilder(
                      future: _gelirService.getCategoriesForUser(
                          userId, "${Apis.getGelirCategoriesApi}2"),
                      builder: (BuildContext context,
  ...
  Future<void> _showDeleteSubCategoryDialog(int userId) async {
    ...
    return showDialog(
      context: context,
      builder: (BuildContext context) => AlertDialog(
        title: const Text("Gıda Türü Kaldır"),
        content: SingleChildScrollView(
          child: Padding(
            padding: const EdgeInsets.all(8),
            child: Form(
            ...
            onPressed: () {
              setState(() {
                _gelirService.deleteSubCategory(context, userId, selectDataItem,
                    2 /*categoryId*/, Apis.deleteGelirSubCategoriesApi);
              });
            },
  ...
  Future<void> _showAddSubCategoryDialog(int userId) async {
    ...
    return showDialog(
        context: context,
        builder: (BuildContext context) => AlertDialog(
          title: const Text('Gıda Türü Ekle'),
          content: SingleChildScrollView(
            child: Padding(
              padding: const EdgeInsets.all(8),
              child: Form(
                ...
                child: TextFormField(
                  ...
                  validator: (String? value) {
                    if (value == null || value.isEmpty) {
                      return 'Lütfen Geçerli bir Gıda Türü giriniz';
                    }
                    return null;
                  },
                  onChanged: (value) {
            ...
            TextButton(
                onPressed: () {
                  _gelirService.saveSubCategory(context, userId, gelirType,
                      Apis.saveGelirSubCategoriesApi, 2 /*categoryId*/);
                },
                child: const Text('Kaydet')),
            TextButton(
                onPressed: () {
