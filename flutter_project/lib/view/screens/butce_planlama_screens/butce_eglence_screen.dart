...
class ButceEglence extends StatefulWidget {
  const ButceEglence({Key? key}) : super(key: key);

  @override
  State<StatefulWidget> createState() => ButceEglenceState();
}

class ButceEglenceState extends State<ButceEglence> {
                ...
                SizedBox(
                  ...
                  child: FutureBuilder(
                    future: _gelirService.getData(
                        Apis.getGelirApi, "7", startDate, endDate),
                              ...
                              child: InkWell(
                                onTap: () {
                                  _gelirWidget.showGelirDetails(context,
                                      document.id, "7", startDate, endDate);
                                },
                                child: Row(
                                  children: [
                                    Padding(
                  ...
                  children: <Widget>[
                    FutureBuilder(
                      future: _gelirService.getCategoriesForUser(
                          userId, "${Apis.getGelirCategoriesApi}7"),
                      builder: (BuildContext context,
  ...
  Future<void> _showDeleteSubCategoryDialog(int userId) async {
    ...
    return showDialog(
      context: context,
      builder: (BuildContext context) => AlertDialog(
        title: const Text("Eğlence Türü Kaldır"),
        content: SingleChildScrollView(
          child: Padding(
            padding: const EdgeInsets.all(8),
            child: Form(
            ...
            onPressed: () {
              setState(() {
                _gelirService.deleteSubCategory(context, userId, selectDataItem,
                    7 /*categoryId*/, Apis.deleteGelirSubCategoriesApi);
              });
            },
  ...
  Future<void> _showAddSubCategoryDialog(int userId) async {
    ...
    return showDialog(
        context: context,
        builder: (BuildContext context) => AlertDialog(
          title: const Text('Eğlence Türü Ekle'),
          content: SingleChildScrollView(
            child: Padding(
              padding: const EdgeInsets.all(8),
              child: Form(
                ...
                child: TextFormField(
                  decoration: const InputDecoration(
                    hintText: 'Eğlence Türü',
                  ),
                  validator: (String? value) {
                    if (value == null || value.isEmpty) {
                      return 'Lütfen Geçerli bir Eğlence Türü giriniz';
                    }
                    return null;
                  },
                  onChanged: (value) {
            ...
            TextButton(
                onPressed: () {
                  _gelirService.saveSubCategory(context, userId, gelirType,
                      Apis.saveGelirSubCategoriesApi, 7 /*categoryId*/);
                },
                child: const Text('Kaydet')),
            TextButton(
                onPressed: () {
