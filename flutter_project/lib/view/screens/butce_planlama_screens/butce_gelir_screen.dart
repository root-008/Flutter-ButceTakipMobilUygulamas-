...
class ButceGelir extends StatefulWidget {
  const ButceGelir({Key? key}) : super(key: key);

  @override
  State<StatefulWidget> createState() => ButceGelirState();
}

class ButceGelirState extends State<ButceGelir> {
                ...
                SizedBox(
                  ...
                  child: FutureBuilder(
                    future: _gelirService.getData(
                        Apis.getGelirApi, "1", startDate, endDate),
                              ...
                              child: InkWell(
                                onTap: () {
                                  _gelirWidget.showGelirDetails(context,
                                      document.id, "1", startDate, endDate);
                                },
                                child: Row(
                                  children: [
                                    Padding(
                      ...
                      children: <Widget>[
                        FutureBuilder(
                          future: _gelirService.getCategoriesForUser(
                              userId, "${Apis.getGelirCategoriesApi}1"),
                          builder: (BuildContext context,
  ...
  Future<void> _showDeleteSubCategoryDialog(int userId) async {
    ...
    return showDialog(
      context: context,
      builder: (BuildContext context) => AlertDialog(
        title: const Text("Gelir Türü Kaldır"),
        content: SingleChildScrollView(
          child: Padding(
            padding: const EdgeInsets.all(8),
            child: Form(
            ...
            onPressed: () {
              setState(() {
                _gelirService.deleteSubCategory(context, userId, selectDataItem,
                    1 /*categoryId*/, Apis.deleteGelirSubCategoriesApi);
              });
            },
  ...
  Future<void> _showAddSubCategoryDialog(int userId) async {
    ...
    return showDialog(
        context: context,
        builder: (BuildContext context) => AlertDialog(
              title: const Text('Gelir Türü Ekle'),
              content: SingleChildScrollView(
                child: Padding(
                  padding: const EdgeInsets.all(8),
                  child: Form(
                    ...
                    child: TextFormField(
                      ...
                      validator: (String? value) {
                        if (value == null || value.isEmpty) {
                          return 'Lütfen Geçerli bir Gelir Türü giriniz';
                        }
                        return null;
                      },
                      onChanged: (value) {
                ...
                TextButton(
                    onPressed: () {
                      _gelirService.saveSubCategory(context, userId, gelirType,
                          Apis.saveGelirSubCategoriesApi, 1 /*categoryId*/);
                    },
                    child: const Text('Kaydet')),
                TextButton(
                    onPressed: () {
